...
# 新功能追加
- 新增一個頁面：使用者可以追加 youtube 頻道（可以一次丟多個），你會將該頻道資訊抓進來並塞進 `eroge_brands` 內，但是新加入的頻道預設為 `未核准` 。狀態需要經過管理人我確認後，放行為核准時才可以使用。因此狀態在 `未核准` 時不會針對該頻道去抓取影片進入索引。資料表名稱一樣使用 `eroge_` 開頭

# 新功能追加
- 把待審頻道功能搬進 `/admin`  開頭路由
- 「一樣使用 `/admin` 」加入既有頻道的管理：需要加上搜尋。可以對頻道進行以下操作：
    分成三個 `Tab`： `全部影片` 可以操作 `刪除` 與 `暫停索引`。 `已刪除頻道`：可執行 `刪除回滾` 。 `暫停索引頻道`：可執行 `重新啟動索引`
    - 刪除：一旦刪除後就將頻道以及所屬影片從 ES 移除（DB 採 soft delete），相關操作使用背景處理。之後就算重跑所有頻道索引更新也不會跑。另外使用者若是收藏該頻道或是其所屬影片則將區塊改成 disabled 。
    - 刪除回滾：根據 DB 狀態，rollback 頻道及影片資訊，相關操作使用背景處理。
    - 暫停索引：僅暫停更新頻道及其所屬影片進 ES。該頻道仍可被讀取。但不會有新影片進來。
    - 重新啟動索引：將頻道更改為可索引，並開始執行從最後一次執行索引時間之後的索引。
- 「一樣使用 `/admin` 」加入既有影片管理：
    - 刪除影片：DB 使用 Soft Delete，從 ES 拔掉該影片索引
- 增加追加影片功能：使用者可以貼入 youtube 網址，e.g. `youtu.be/xxxxx` `youtube.com/watch?v=xxxx` 。若是頻道未被加入過其所屬頻道進入待審頻道後，其影片才能審查是否被加入。考慮到之後追加的量可能會很大，因此覺得使用一個專用資料表儲存
- 「一樣使用 `/admin` 」由於增加追加影片功能，因此也需要增加一個待審影片功能，實作前述的功能