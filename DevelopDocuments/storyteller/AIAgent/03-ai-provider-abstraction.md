...
**已知缺口**：`storyteller_agent_models`（`model/entity/storyteller/storyteller.go:345`）目前只存 name/label/description/price，沒有存每個 model 的 output cap／是否支援 reasoning 之類的 capability metadata，之後要做「依 model 自動 clamp 上限」或選單會需要先補這塊。Gemini thinking model 在 function-calling 回合要求回放的 `thoughtSignature` 已透過 `ToolCall.ThoughtSignature` 承載，`GeminiProvider` 會在下一輪 history 的 `functionCall` part 原樣帶回；但若之後要完整保留 provider-native reasoning/thought blocks（例如非 tool-call part 上的 signature 或可視化 thought summary），共用 `Message` 仍只有純文字跟 tool call，需要再設計更完整的 content part 抽象。
