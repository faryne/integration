...
### 8. ~~真實登入頁面上，slash 選單每個項目前面的 icon 完全不見~~ ✅ **已修（2026-08-19，開始 Phase E 前先處理）**
...
- **已排除的可能性**（原始記錄）：不是有兩份不同元件、不是 build 方式不同、不是 HMR 沒吃到最新程式碼、console 沒有相關錯誤。
- **當時唯一還沒證實的方向**：icon 原本透過 `renderCommandIconMarkup()` 呼叫 `renderToStaticMarkup(createElement(icon, {fontSize:"small"}))` 動態產生 SVG 字串、再用 `innerHTML` 塞進手刻的 DOM——這整段是在 React tree 之外執行的（不是 JSX 渲染），懷疑跟真實頁面被 `ThemeProvider`/`cssVariables:true` 包住有關（MUI icon 元件內部 `useTheme()` 在脫離正常 React tree 的獨立同步渲染裡拿不到外層 context）。
- **Root cause／解法**：Faryne 提出「開始 Phase E（無障礙功能 audit）之前先處理這個，順便讓 slash 選單跟右鍵選單的實作方式一致，避免之後補 a11y 屬性時兩邊各改一次、加 A 少 B」。順著這個方向重新檢視，找到真正的問題：`renderToStaticMarkup` 是**伺服器端渲染**用的同步字串化函式，在客戶端呼叫本來就不是 Tiptap／React 官方支援的路徑；`@tiptap/suggestion` 其實內建了正規的解法——`SuggestionProps.mount()` 搭配 `ReactRenderer`（`@tiptap/react` 提供），會把內容透過 portal 掛進 editor 元件自己所在的那棵 React tree，而不是憑空產生一段跟 App 無關的 DOM/字串。改用這個方式後：
  - `slashCommandExtension.tsx` 整個手刻 DOM 渲染邏輯（`renderCommandIconMarkup`／`renderSlashCommandItems`）拿掉，改成一個真正的 React 元件 `SlashCommandList`（MUI `Paper` + 按鈕列表，icon 直接當 JSX `<Icon fontSize="small" />` 渲染，不再有任何字串拼接），透過 `new ReactRenderer(SlashCommandList, {editor, props})` 建立、`props.mount(renderer.element)` 掛載、`renderer.updateProps()` 更新選取狀態、`renderer.destroy()` 收尾——寫法跟右鍵選單／bubble menu 一樣是「消費同一份 command registry、用 JSX 畫 icon+label」，不再是三套不同的實作風格。
  - 鍵盤導覽（上下鍵切換、Enter 選取、Escape 關閉、IME 組字期間暫停攔截）邏輯完全沒動，只是原本直接操作 DOM 的地方改成呼叫 `renderer.updateProps()`。
- **驗證**：`npx tsc -b --noEmit` 乾淨、`npx vitest run` 43/43 通過（既有的 `slashCommandExtension.test.ts` 只測 `canShowSlashCommand`／`runSlashCommand`，不碰 renderer 內部，這次改動沒有動到）。Playground 實測：icon 正常顯示（標題/引用/表格/圖片等都有對應圖示）；鍵盤 Enter 選取、滑鼠點擊選取都正確插入內容並清掉 `/query` 文字；console 沒有新增任何錯誤（含刻意測試 `ReactRenderer` 常見的 `flushSync` 警告，開選單/按上下鍵/Enter 選取全程都沒有觸發）。**重現原始 bug 的關鍵條件**（`ThemeProvider` + `cssVariables:true`）：暫時把 Playground 頁面包一層等同 `StorytellerLayout` 的 `ThemeProvider({cssVariables:true})` 驗證（驗證完已還原，未進 commit），確認在這個條件下 icon 依然正常顯示——原本 `renderToStaticMarkup` 版本失效的環境條件，`ReactRenderer` 版本不受影響，符合「掛進 App 自己的 React tree，不再脫離 context」的修法邏輯。因為沒有登入權限，沒有在真正的登入頁面（`localhost:5173/storyteller/my/workspace/...`）複測，麻煩 Faryne 之後找空檔確認一下。
- **細節對齊**：Faryne 複測時發現雖然功能一致，但 slash 選單原本沒有分隔線，跟右鍵選單「標題群組／區塊群組／插入群組」之間有分隔線的排版不一致。`SlashCommandList` 補上：比對相鄰項目的 `command.group` 是否變化，變化就在前面插入 MUI `<Divider />`（不用另外硬寫群組順序或項目數量，query 篩選後不管剩哪些群組都能自動算對交界）。Playground 實測：標題1~6 與「引用」之間、「插入分隔線」與「插入表格」之間都正確出現分隔線，跟右鍵選單的分組視覺一致。
- **追加發現＋根本修正（同一輪）**：Faryne 接著注意到「置左／置中／置右」只出現在右鍵選單、slash 選單沒有，追問兩個選單是不是同一個源頭。查證後發現：command **定義**（icon／label／run 邏輯）兩邊確實共用同一份 `WYSIWYG_COMMANDS`，但「這個選單該顯示哪些 group」是**各自維護一份清單**——slash 選單靠 `commands.ts` 裡硬寫的 `SLASH_COMMAND_GROUPS` 常數，右鍵選單則是在 `StorytellerWysiwygContextMenu.tsx` 的 JSX 裡手動列出三段幾乎一樣的 `wysiwygCommandsByGroup("heading"/"align"/"block")`。2026-08-16 補 align 進右鍵選單時只改了後者，沒有同步 `SLASH_COMMAND_GROUPS`，兩邊因此悄悄長出落差，且沒有任何機制會在下次新增 group 時提醒要兩邊都改。
  - **修法**：在 `commands.ts` 新增匯出常數 `BLOCK_OPERATION_GROUPS = ["heading", "align", "block"]`，作為「無選取時的區塊操作分組」的唯一真相來源；`SLASH_COMMAND_GROUPS` 改成 `[...BLOCK_OPERATION_GROUPS, "insert"]`，不再獨立硬寫。右鍵選單的「無選取」分支原本三段各自 `.map()` 渲染 heading/align/block（含各自的 `<Divider>`），改成 `BLOCK_OPERATION_GROUPS.flatMap()` 單一迴圈、group 交界處自動插入分隔線——跟 slash 選單這次新加的分隔線邏輯是同一招。之後不管是新增 group 還是調整順序，只要改 `BLOCK_OPERATION_GROUPS` 這一個地方，兩邊選單會一起同步，不會再需要記得改兩處。
  - **驗證**：`npx tsc -b --noEmit` 乾淨、`npx vitest run` 43/43 通過。Playground 實測：slash 選單 `/置中` 能篩到「置中」並正確套用 `textAlign:"center"`；右鍵選單「無選取」分支視覺跟改動前逐項比對一致（標題1~6 → 分隔線 → 置左/置中/置右 → 分隔線 → 引用/清單/分隔線 → 分隔線 → 插入表格），確認拆成共用迴圈沒有造成視覺或行為差異；console 無新增錯誤。
- **第二次真實頁面回報＋改用 `createRoot`**：Faryne 在真實編輯頁面（非 Playground）複測時回報：打 `/` 之後選單完全叫不出來（連文字都沒有，不是 icon 消失而已，比原本的 bug 更嚴重）。因為沒有登入權限，沒辦法直接在真實頁面重現，改回頭檢查 `ReactRenderer` 的原始碼（`@tiptap/react` 套件的 `dist/index.js`）找可能的環境落差來源，找到關鍵一行：`ReactRenderer.render()` 內部是呼叫 `editor.contentComponent?.setRenderer(this.id, this)` 才真正把內容掛進 DOM——`contentComponent` 是 `@tiptap/react` 的 `EditorContent` 元件掛載時才會設定好的內部狀態，`?.` 代表如果這個時間點 `contentComponent`還沒就緒或不存在，`setRenderer` 呼叫會被靜默吞掉，`ReactRenderer` 什麼都不會渲染、也不會拋錯——跟 Faryne 回報的「完全沒東西、無錯誤訊息」症狀完全吻合。真實頁面（`StoryEditor.tsx`／`LoreEditor.tsx`，有登入態、資料載入、autosave 等額外流程）跟 Playground（内容直接寫死、沒有異步資料流程）的 `EditorContent` 掛載時序顯然不同，導致這個內部依賴在真實頁面沒有正確就緒。
  - **改法**：不再依賴 `ReactRenderer`／`editor.contentComponent` 這個內部整合點，改用最基本的 `react-dom/client` 的 `createRoot()` 建立一個完全獨立、自給自足的 React root 掛在 `props.mount()` 給的 DOM 節點上——這是任何 React 應用都能用的標準 API，不依賴 Tiptap 或 `EditorContent` 的任何內部狀態，等於是「舊版手刻 DOM 方案的可靠性」＋「新版真正 JSX icon」的組合：色彩一樣吃 `var(--storyteller-*)` CSS variable（掛在 `:root` 上，任何 DOM 節點都讀得到，不需要 React context／ThemeProvider），不會因為獨立 root 拿不到 `ThemeProvider` context 而跑版。掛載當下用 `flushSync` 同步 render 一次，確保 `props.mount()` 量測位置時 element 已經有真實尺寸；上下鍵切換高亮則不特別要求同步。
  - **驗證**：`npx tsc -b --noEmit` 乾淨、`npx vitest run` 43/43 通過。Playground 重新測過一輪（含 `ThemeProvider(cssVariables:true)` 包裹的驗證條件）：icon 正常顯示、鍵盤/滑鼠選取都正確運作、console 無新增錯誤。**這一版同樣沒有登入權限可以在真正的登入頁面複測**——但這次的修法不再依賴任何「只有在特定掛載時序才會就緒」的內部狀態，理論上排除了造成這次回報的那個失效條件；麻煩 Faryne 之後找空檔確認一下 slash 選單能不能正常叫出來、icon 有沒有顯示。
- **第三次真實頁面回報＋真正的 root cause（z-index）**：`createRoot` 版本上線後，Faryne 在真實編輯頁面複測仍回報「打 `/` 完全叫不出來」，跟改用 `createRoot` 之前一樣。這次沒辦法再靠讀原始碼猜測，改用 Faryne 遠端提供的瀏覽器 console 資訊逐步縮小範圍（因為沒有登入權限，這是唯一能拿到真實頁面 DOM 狀態的管道）：
  1. 打 `/` 之後 console 完全沒有任何錯誤或警告（排除「丟例外被靜默吞掉」的可能性，`createRoot`／`root.render()` 本身沒有出錯）。
  2. 請 Faryne 執行 `document.querySelectorAll('[role="listbox"]').length`，回傳 `1`——選單**有**掛進 DOM（排除「根本沒有 mount」的可能性，`props.mount()` 有被呼叫、`createRoot` 有成功渲染）。
  3. 請 Faryne 進一步執行 `getBoundingClientRect()` 跟 `getComputedStyle()`，結果顯示：位置尺寸合理（`x:301 y:267 width:180 height:268`，不是 0 或跑到螢幕外）、`childCount:16`（16 個項目都真的渲染出來了）、`display:block`／`visibility:visible`／`opacity:1`（不是被隱藏）——**但 `zIndex` 讀出來是 `"auto"`**。
  找到真正的 root cause：舊版手刻 DOM 的實作原本有明講 `z-index:1500`（真實頁面有 sticky 置頂的標題列等元素會搶堆疊順序，見本文件其他段落提到的 `StoryEditor.tsx`／`LoreEditor.tsx` 的 `embeddedHeaderContent`，`zIndex:2`），這次重寫成 `SlashCommandList`／MUI `Paper` 時把這個 `z-index` 設定漏掉了，`Paper` 預設沒有 z-index。選單雖然正確掛進 DOM、位置尺寸內容都對，畫面上卻被同頁面其他有明講 z-index 的元素蓋住看不見。Playground 沒有 sticky header 這類會搶堆疊順序的元素，才會誤以為「正常」，兩輪都沒踩到。
  - **第一次修法（有誤）**：把 `zIndex: 1500` 設在 `SlashCommandList` 的 `Paper` 上。Faryne 複測後回報「一樣沒看到 menu」，重新請她跑一次同一段診斷 script，這次 `zIndex` 讀出來確實是 `"1500"`，但畫面上還是看不到——代表這個 z-index 設定沒有真的生效。
  - **第二次修法（真正生效）**：想到一個先前漏掉的 CSS 規則——**z-index 只對「有明確 `position`（relative/absolute/fixed/sticky）」的元素有效，對 `position:static` 的元素完全沒作用**。`Paper` 預設就是 `position:static`，設 `zIndex` 完全被瀏覽器忽略；真正決定「這個選單在整個頁面堆疊順序排第幾層」的，是 `props.mount()` 掛載用的那個外層 wrapper `<div>`（診斷資料裡 `wrapperPosition:"absolute"` 那個，`@tiptap/suggestion` 會把它設成 `position:absolute` 來做浮動定位）——這個 wrapper 是我們自己 `document.createElement("div")` 建立、直接傳給 `props.mount()` 的節點，可以直接在建立時用 `element.style.zIndex = "1500"` 設定。改到這裡才是真正會生效的位置；`Paper` 上的 `zIndex` 設定拿掉（留著也沒用，只會誤導後續讀 code 的人以為那裡才是修好的地方）。
  - **驗證**：`npx tsc -b --noEmit` 乾淨、`npx vitest run` 43/43 通過。Playground 重新確認：`getComputedStyle(wrapper).zIndex === "1500"`（wrapper 本身，不是選單內容）、選單正常顯示；另外測過「選單開著時點擊項目關閉」「開著時按 Escape 關閉」兩種 unmount 路徑，console 都沒有錯誤（用全新分頁測試排除掉先前快速連續操作留下的殘留 console 訊息干擾）。**這兩輪修法都無法在 Playground 重現原始問題**（Playground 本來就沒有 z-index 衝突），完全靠 Faryne 提供的真實頁面 DOM 診斷資料一步步反推——第一次診斷（`zIndex:"auto"`）指出方向但沒指出精確位置，導致第一次修法補錯地方；第二次診斷（補完 z-index 後仍看不到，但 `zIndex` 讀出來確實是 `"1500"`）才逼著往下追出「z-index 只對 positioned 元素有效」這條 CSS 規則。已請 Faryne 再次確認最終效果。
- **細節對齊（分隔線顏色）**：Faryne 確認選單能正常叫出來後，比對右鍵選單發現一個不容易注意到的差異——slash 選單的分隔線在深色模式下幾乎看不見（顏色偏黑），右鍵選單的分隔線是灰色、看得清楚。原因：MUI `<Divider>` 沒有另外指定顏色時吃 `theme.palette.divider`；右鍵選單在 `StorytellerLayout` 的 `ThemeProvider` 底下，讀得到 storyteller 主題明講的 `divider: tokens.border`（偏灰），但 slash 選單是獨立的 `createRoot`，沒有包在任何 `ThemeProvider` 裡，`<Divider>` 只能退回 MUI 內建預設值（低透明度的黑），在深色背景上幾乎融進背景。跟選單其他顏色（背景／文字／選取高亮）已經在用的做法一致，補上 `sx={{ borderColor: "var(--storyteller-border-subtle, ...)" }}`，不依賴 theme context。驗證：`npx tsc -b --noEmit`／`npx vitest run` 43/43 通過；因為 Playground 本身沒有掛 `--storyteller-*` 變數，用 `document.documentElement.style.setProperty()` 手動注入一個測試色值，確認分隔線的 `getComputedStyle().borderBottomColor` 精確等於注入值，證明改吃 CSS variable 而不是 MUI 預設值。
- **狀態**：已修（歷經五輪：icon 消失→`ReactRenderer` 版本在真實頁面完全叫不出來→`createRoot` 版本改善了掛載可靠性但漏掉 z-index→z-index 設錯位置（設在 `position:static` 的選單內容上，沒有效果）→改設在 `position:absolute` 的 wrapper `<div>` 上才叫得出來→分隔線顏色沒吃到 theme、深色模式看不見，改吃 `var(--storyteller-border-subtle)` 才是最終完成）。slash 選單跟右鍵選單現在共用同一份「無選取區塊操作」group 清單（`BLOCK_OPERATION_GROUPS`），也共用同一套「JSX 畫 icon+label＋group 交界自動分隔線」的實作風格；slash 選單本身改用不依賴 Tiptap 內部整合狀態的 `createRoot` 掛載方式，堆疊順序跟顏色都改在正確的節點／來源上設定（不依賴 `ThemeProvider` context）。為 Phase E 要補的鍵盤導覽/aria 屬性打底——之後新增 group 只要改一個地方，不會再有「這邊補了那邊忘記」的落差。**這次的教訓**：把一段原本在 `ThemeProvider` context 之外運作的手刻 DOM 改寫成「看起來像」正常 React／MUI 元件時，容易忽略它其實還是脫離 context 的獨立 root——任何沒有明講 `var(--storyteller-*)`、依賴 MUI 預設 theme 解析的顏色/樣式，都要逐一檢查是不是不小心引用了「context 才有」的值；這次踩到兩次（z-index 位置、divider 顏色），都是同一類疏漏的不同面向。
...
- **現象**：Faryne 確認第 14 項圖片 Backspace 修好後，接著測試「圖片後接引用」（Phase G 項目 1）發現連帶解決了——插入引用會直接跳到圖片下方，不再重疊。但測試過程中發現新問題：把一個引用行的文字用 Backspace 刪光後，那一行看起來還是一個空的引用框（沒有變回普通段落）；在這個空引用行上再按一次 Backspace，會刪掉**前一行**的文字，但引用框本身還在，畫面變得很不乾淨。
- **Root cause**：「空白的清單項目/引用行按 Enter 會跳出格式（變回一般段落）」這個邏輯只綁在 `Enter` 上（見同一份文件已知 Bug 記錄，`markerParagraph.ts` 的 `Enter` handler），`Backspace` 完全沒有對應處理。使用者用 Backspace 把引用行文字一個字一個字刪光後，段落的 `blockKind` 屬性沒有任何機制會自動重置——ProseMirror 本身不知道「文字刪空了」跟「使用者想跳出引用格式」有什麼關聯，這純粹是這個自訂 schema 才有的語意，需要額外程式碼處理。於是這一行變成「文字是空的，但 `blockKind` 還是 `quote`」，畫面照樣渲染成一個空的引用框。接著在這個空引用行按 Backspace（游標在段落開頭），會直接落到 ProseMirror 對「兩個 blockKind 不同的段落合併」的預設處理路徑——這個路徑不是這個自訂 schema 特別測試過的情境，合併結果不穩定（觀察到的現象是刪掉前一行文字、引用框卻沒被移除）。
- **解法**：在 `Backspace` handler 最前面加一段，跟 Enter 的邏輯對稱：游標在段落開頭（`parentOffset === 0`）、目前段落是空白清單/引用行（`blockKind !== "none"` 且文字內容為空）時，第一次 Backspace 只把 `blockKind` 重置回 `"none"`（變回一般段落），不做任何合併——比照 Enter 已經有的「空項目跳出格式」慣例。跳出格式之後，這一行就是普通的空白段落，使用者再按第二次 Backspace 時，走的就是「兩個一般段落合併」這條穩定、有大量既有使用經驗的路徑，不會再撞到「不同 blockKind 合併」的不穩定情境。
