...
- **狀態**：已修。slash 選單跟右鍵選單現在共用同一份「無選取區塊操作」group 清單（`BLOCK_OPERATION_GROUPS`），也共用同一套「JSX 畫 icon+label＋group 交界自動分隔線」的實作風格，為 Phase E 要補的鍵盤導覽/aria 屬性打底——之後新增 group 只要改一個地方，不會再有「這邊補了那邊忘記」的落差。
