...
- **狀態**：已修（歷經四輪：icon 消失→`ReactRenderer` 版本在真實頁面完全叫不出來→`createRoot` 版本改善了掛載可靠性但漏掉 z-index→z-index 設錯位置（設在 `position:static` 的選單內容上，沒有效果）→改設在 `position:absolute` 的 wrapper `<div>` 上才是真正解決）。slash 選單跟右鍵選單現在共用同一份「無選取區塊操作」group 清單（`BLOCK_OPERATION_GROUPS`），也共用同一套「JSX 畫 icon+label＋group 交界自動分隔線」的實作風格；slash 選單本身改用不依賴 Tiptap 內部整合狀態的 `createRoot` 掛載方式，並在正確的節點（wrapper，而非內容本身）補回堆疊順序設定。為 Phase E 要補的鍵盤導覽/aria 屬性打底——之後新增 group 只要改一個地方，不會再有「這邊補了那邊忘記」的落差。**這次的教訓**：手刻 DOM 改寫成元件化實作時，容易漏掉一些「看起來像裝飾、實際上是特定環境才需要」的細節（這次是 z-index），且就算想到要補，也要想清楚「這條 CSS 規則實際生效的節點是哪一個」——z-index 只對有明確 `position` 的元素有效，設在錯的節點上（哪怕數值完全正確）一樣不會生效，光靠肉眼比對「看起來一樣」的視覺結果並不足夠，必須配合 `getComputedStyle` 之類的診斷資料實際確認。
