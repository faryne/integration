...
- [ ] 舊 `table-row` 資料相容：parser 保留讀取能力，新增內容不再產生 `table-row`，提供手動「轉換成新表格」command，不靜默自動轉——2026-08-14 Codex 已移除新建入口（`| ` input rule、`block-kind-table-row` command、語法 drawer 內的舊表格列教學），舊資料 parser/reader/export 相容仍保留；已補 `convertLegacyTableRowsToStorytellerTable()` core command 與測試，會把游標所在的連續舊 `table-row` 段落轉成真表格、保留行內 mark、欄數不一致時補空 cell。Codex 本地 `tsc -b --noEmit` 通過，但 command 行為的 jsdom/vitest smoke 仍因 sandbox 的 `SecItemCopyMatching failed -50` 失敗，待 Claude/reviewer 跑過正式 `vitest` 後再打勾
