...
- [ ] 人工實測中文 IME：輸入 `/標`、注音組字期間 suggestion menu 的互動、Escape/Enter 行為——已用瀏覽器自動化對「已組字完成」文字做過逐字元真實按鍵測試（ArrowDown／Enter／Escape／滑鼠都確認正確），但這不等於「注音/拼音組字中途」的行為，自動化工具無法模擬真正的 IME composition 狀態，維持未勾，跟其他 IME 高風險項一起排到 Phase 9（見 Phase 9.1 測試案例）
...
- [ ] StoryEditor／LoreEditor 實測：autosave、字數、AI agent 附加、匯出 markdown 不受影響——2026-08-16 這項需要真實登入帳號＋真實 story/lore API 操作，Claude/Codex 的環境都沒有這個條件，不去借用使用者的登入 session。改用程式碼審查型驗收：`git diff` 確認整個 Phase 6（108f2ff..6e1af0e）`StoryEditor.tsx`／`LoreEditor.tsx` 完全零改動；`StorytellerWysiwygEditorHandle`／`useImperativeHandle`（`insertAsset` method）介面沒變；`toolbarExtra` prop 仍有 render，只是位置搬到右上角文件層級 action 區；autosave／字數／AI agent 附加這些邏輯都是頁面層自己的 `useEffect`／API 呼叫，跟被拔掉的格式工具列 JSX 完全獨立，程式碼審查沒有發現風險。真實頁面的人工操作驗收（改字、確認 autosave 訊息與刷新後內容保留、字數更新、AI 面板附加內容同步、匯出 markdown 下載、右上角插入資產按鈕）留給使用者自己找時間做，併入 Phase 9.2（附完整測試步驟），不現在打勾
...
2026-08-16 新增。以下每一項都是 Claude／Codex 已確認無法透過程式或瀏覽器自動化驗證的項目（原因見各項說明），需要使用者用真實鍵盤/輸入法/帳號/裝置親自操作一次。每個測試案例都寫成「照著做、看結果符不符合預期」的具體步驟，不需要自己想怎麼測。測完可以直接回報「哪幾項過、哪幾項有問題」，不用整份都測完才回報，發現問題可以隨時反饋，不用等全部做完。

#### 9.1 中文 IME 組字測試（風險最高，建議優先測）
...
1. 在編輯器空白處，開始用注音/拼音打一段中文字（例如打「測試」，但先不要按 Enter/空白選字，讓候選字視窗保持開著）
2. 在候選字視窗開著、還沒選字完成的狀態下，按看看數字鍵/符號鍵切換候選字，確認畫面沒有誤觸發任何自動轉換（例如不會突然冒出粗體/刪除線效果）
3. 完成選字後，再實際打以下幾組符號並確認格式有正確自動套用：
   - `**文字**` → 應變粗體
   - `*文字*` → 應變斜體
   - `++文字++` → 應變底線
   - `~文字~` → 應變下標
   ...
   - `--文字--` → 應變刪除線
   - 獨立一行打 `---` → 應變成分隔線
4. 重點：這些符號如果是在中文組字「中途」按到（例如打注音候選字時剛好選到某個符號），要確認不會誤觸發格式轉換；組字完成、符號是你真的要打的那幾個字元時才應該轉換
5. **預期結果**：中文正常輸入、候選字正常運作；符號只有在你真正打完整組（例如兩個 `*`）而且不是在組字中途時才會轉換成對應格式
6. 如果有任何一種符號沒有按預期轉換、或誤在組字中途轉換，記錄下：用的是哪種輸入法（注音/拼音/其他）、打了什麼、發生了什麼

**案例 2：`/` slash command 選單與組字互動**
1. 在空白段落，用輸入法打「/標」（斜線加注音一個字，例如打「/」再打「標」這個字，中途讓候選字視窗開著）
2. 觀察 slash 選單在候選字視窗開著的狀態下會不會正常顯示、有沒有跑版或抖動
3. 選字完成後，確認選單內容有沒有正確篩選出「標題」相關選項
4. 測試組字期間按 Escape：確認是取消輸入法候選字，還是不小心關掉了 slash 選單（預期應該是先關輸入法候選字，不影響 slash 選單）
5. 測試組字期間按 Enter：確認是完成選字，還是不小心觸發了 slash 選單裡的項目
6. **預期結果**：中文輸入法的候選字操作（選字、Escape 取消）跟 slash 選單的操作不會互相干擾、誤觸發
7. 記錄任何不符預期的行為（發生時機、按了什麼鍵、結果是什麼）

**案例 3：表格 cell 內中文組字**
1. 用 `/table` 或右鍵插入一張真表格
2. 點進任一個 cell，用輸入法打一段中文（組字中途候選字視窗開著時，確認畫面顯示正常、沒有跑到別的 cell 去）
3. 打完後用 Tab／Shift-Tab 切換到別的 cell，確認切換不會打斷正在進行的組字（如果剛好在組字中途按 Tab，建議也測一次，確認候選字不會殘留或消失文字）
4. **預期結果**：cell 內中文輸入跟一般段落一樣順暢，Tab 切換不會弄丟文字或打斷輸入
5. 記錄任何異常
...
1. 打開任一個故事（或設定集）的編輯頁面
2. 確認右上角看得到一個小的 action 區（語法說明「？」icon、匯出 icon、插入資產/AI Agent/編輯歷史相關按鈕），確認**沒有**看到舊的格式工具列（不會有一整排粗體/斜體/標題下拉那種橫向工具列）
3. 在內文隨便改一小段文字（例如加一句話）
4. 等幾秒，確認畫面上有出現 autosave 相關的提示（存檔中/已儲存之類的訊息，跟你熟悉的既有行為一致）
5. 重新整理整個頁面，確認剛剛改的內容還在（代表真的存檔成功，不是只在畫面上而已）
6. 看一下字數顯示的地方，確認數字有跟著你剛剛加的文字更新
7. 點開 AI Agent 面板（右上角 action 區的機器人 icon），試著用 AI 附加/插入一段內容，確認附加的內容有正確同步進編輯器內文
8. 點右上角的匯出圖示，確認會下載一個 `.md` 檔案，打開檔案確認內容看起來合理（沒有亂碼、沒有殘留 `⟦⟧` 這種內部標記語法外洩）
9. 點右上角插入資產的按鈕（或在空白段落打 `/圖片`），確認資產選擇視窗能正常開啟、選圖後能正確插入
10. **預期結果**：以上每一步都跟你印象中「拔工具列之前」的行為一致，只是操作入口從工具列變成右上角 action 區／右鍵選單／`/` 指令／選取文字後的浮動選單
11. 任何一步跟你預期不一樣（消失的功能、跑版、存檔失敗等），直接記錄下來，附上是 StoryEditor 還是 LoreEditor、做到第幾步發現的
...
**測試步驟**：
1. 插入一張真表格（`/table` 或右鍵選單）
2. 準備一段純文字（例如從記事本複製一句話），點進某個 cell，貼上（Cmd+V）
   - 預期：文字只會進到你點的那個 cell 裡，不會跑到別的 cell 或跑出表格外
3. 準備一段「有換行」的多行文字（例如複製三行不同的句子），點進某個 cell 貼上
   - 觀察會發生什麼：可能全部擠進同一個 cell（用換行符號連接或直接接在一起），也可能出現非預期的行為（例如表格結構跑掉、多出新的段落）
   - 記錄實際發生的狀況，這項目前沒有標準答案，重點是「有沒有把文件搞壞」
4. 打開一個試算表軟體（Excel、Google Sheets、Numbers 都可以），選幾個相鄰的儲存格（例如 2 欄 x 2 列），複製，貼到表格的某個 cell 裡
   - 觀察：這種貼上內容通常帶有 tab 字元分隔欄位、換行分隔列，目前的編輯器**沒有**特別處理這種格式（不會自動展開成多個 cell），預期比較可能是整段內容擠進同一個 cell、tab 字元可能會消失或變成空白
   - 重點一樣是「有沒有把文件搞壞」，不是「有沒有自動展開成表格」（這是第一版明確不做的功能）
5. 選取表格內多個相鄰 cell（拖曳選取，畫面應該會出現藍色的 cell 選取範圍），再貼上一段文字
   - 觀察：可能只有其中一個 cell 收到內容，也可能有其他行為，記錄實際狀況
6. **預期結果**：以上情境「不要求」貼上內容變成合理的多欄表格（這不是第一版承諾的功能），但**要求**貼上後表格結構不能壞掉（不會整張表消失、不會跑出無法編輯的錯誤畫面），文件其他部分也不受影響
7. 如果發現貼上後畫面卡住、表格結構明顯損壞、或跳出任何錯誤訊息，記錄下：貼的是什麼內容（純文字/多行/試算表複製/多 cell 選取哪一種）、發生了什麼
...
1. 確認畫面排版正常，看得到右上角的小 action 區，沒有橫向格式工具列
2. 手指點一下空白段落，確認有沒有跳出鍵盤，並確認能不能看到 placeholder 提示文字
3. 打 `/` 試試看 slash 選單會不會跳出來、觸控點選選單項目是否順暢
4. 選取一段文字（長按拖曳選取範圍），確認選取文字後有沒有跳出 bubble menu（浮動小工具列），點裡面的按鈕（例如粗體）確認能正常套用
5. 嘗試長按段落，看看有沒有辦法叫出右鍵選單的替代方案（手機瀏覽器通常用長按觸發原生選單，這個編輯器的右鍵選單在手機上可能完全叫不出來，這是預期中的已知限制，重點是確認「除了右鍵以外的入口（slash／bubble menu）是否已經足夠涵蓋主要操作」）
6. 如果有圖片，確認圖片版面（全寬/置中/靠左/靠右環繞）在手機上是否都正確退回全寬顯示，不會有跑版的浮動效果
7. **預期結果**：即使右鍵選單在手機上不可用，slash command 跟 bubble menu 應該已經能完成大部分操作（插入區塊、套用文字樣式）；如果測出某個操作在手機上完全沒有替代入口（例如只能透過右鍵才能做的事），這是需要記錄下來的落差
8. 記錄：用的是什麼手機/瀏覽器、哪個操作在手機上不順或做不到
...
**測試步驟**：
1. 找一段你平常會寫的內容（或直接繼續寫某篇正在寫的故事/設定集），正常寫個幾分鐘，中間穿插你平常會用到的格式（標題、引用、清單、粗體、連結、圖片等）
2. 特別留意：
   - 空白行的 placeholder 提示文字（「輸入 / 插入區塊；選取文字可套用樣式」）出現的時機順不順眼，會不會太常出現、太搶眼、或干擾閱讀
   - 用 `/` 插入區塊操作起來順不順手，跟你以前用工具列比起來是更快還是更麻煩
   - 選取文字後 bubble menu 跳出的位置、時機是否符合直覺
   - 右鍵選單找需要的功能好不好找
3. **預期結果**：這是主觀感受，沒有「對/錯」答案，只要記錄你的實際感覺就好——例如「placeholder 出現得太頻繁，看了很煩」「slash 選單找標題選項要滑很多下」這類具體回饋，之後可以再討論要不要調整
...
**測試步驟**：
1. 找一篇有插圖、內容夠長的故事（或用測試內容），把其中一張圖設成靠左或靠右環繞（右鍵圖片 →「圖片設定」→ 選版面）
2. 用閱讀頁（Reader）實際讀一遍那段內容，感受一下文字環繞圖片的排版讀起來順不順、會不會覺得擠
3. 如果那張圖後面接著標題、引用、表格或另一張圖，確認畫面有沒有跑版、疊在一起的狀況（這項已經自動化驗證過 CSS 正確，這裡是確認實際視覺效果符合預期）
4. 用手機瀏覽器看同一段內容，確認圖片有正確退回全寬（不浮動），排版看起來自然
5. **預期結果**：圖文環繞讀起來自然、沒有明顯跑版；如果覺得某個寬度比例不協調或哪裡讀起來卡卡的，記錄下來（例如「靠左環繞的圖片配短段落文字，環繞範圍太小看起來很怪」），之後可以評估要不要調整第一版的固定寬度比例
