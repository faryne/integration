...
- **狀態**：已修（歷經兩輪，第二輪修正第一輪引入的真實頁面回歸）。slash 選單跟右鍵選單現在共用同一份「無選取區塊操作」group 清單（`BLOCK_OPERATION_GROUPS`），也共用同一套「JSX 畫 icon+label＋group 交界自動分隔線」的實作風格；slash 選單本身改用不依賴 Tiptap 內部整合狀態的 `createRoot` 掛載方式，降低不同環境掛載時序造成落差的風險。為 Phase E 要補的鍵盤導覽/aria 屬性打底——之後新增 group 只要改一個地方，不會再有「這邊補了那邊忘記」的落差。
