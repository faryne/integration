...
修改建議：
- 故事頁及設定頁的「存檔」按鍵是否移到迷你工具列內？避免要手動存檔時還要把頁面拉到開頭按存檔的麻煩（這在行動版頁面的困擾更明顯）。或許搭配 Ctrl+s hotkey 的操作更好？
- 雖然跟這次修改無關：但插入資產的功能是不是可以加說明文字的輸入？並且在閱讀頁顯示該圖片的說明。目前只有替代文字，那算是圖片叉燒包的替代方案。
- 現在工具列完全拔除，但原本工具列上的功能並沒有完全移出來，「文字背景色」就是其中之一。需要確認是否把功能都移進 select / slash menu 或是滑鼠右鍵 menu 中
- slash menu 多個分隔線，把相同群組功能放在一起後使用分隔線分開，可以在之後 createTheme 項目一起處理？
