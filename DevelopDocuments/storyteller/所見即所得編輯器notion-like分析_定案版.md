...
- **第三次真實頁面回報＋真正的 root cause（z-index）**：`createRoot` 版本上線後，Faryne 在真實編輯頁面複測仍回報「打 `/` 完全叫不出來」，跟改用 `createRoot` 之前一樣。這次沒辦法再靠讀原始碼猜測，改用 Faryne 遠端提供的瀏覽器 console 資訊逐步縮小範圍（因為沒有登入權限，這是唯一能拿到真實頁面 DOM 狀態的管道）：
  ...
  - **修法**：`SlashCommandList` 的 `Paper` 補回 `zIndex: 1500`（跟舊版一致的數值）。
  - **驗證**：`npx tsc -b --noEmit` 乾淨、`npx vitest run` 43/43 通過。Playground 重新確認 `getComputedStyle(listbox).zIndex === "1500"`、選單依然正常顯示不受影響。**這次無法在 Playground 重現原始問題**（Playground 本來就沒有 z-index 衝突），修法完全是根據 Faryne 提供的真實頁面 DOM 診斷資料反推得出，屬於這次連續三輪 slash 選單修復裡唯一「純靠使用者遠端診斷、Claude 完全無法自行重現」的一次；已請 Faryne 確認最終效果。
- **狀態**：已修（歷經三輪：icon 消失→`ReactRenderer` 版本在真實頁面完全叫不出來→`createRoot` 版本改善了掛載可靠性，但漏掉 z-index 導致視覺上仍然叫不出來→補回 `zIndex:1500` 才是真正解決）。slash 選單跟右鍵選單現在共用同一份「無選取區塊操作」group 清單（`BLOCK_OPERATION_GROUPS`），也共用同一套「JSX 畫 icon+label＋group 交界自動分隔線」的實作風格；slash 選單本身改用不依賴 Tiptap 內部整合狀態的 `createRoot` 掛載方式並補回堆疊順序設定。為 Phase E 要補的鍵盤導覽/aria 屬性打底——之後新增 group 只要改一個地方，不會再有「這邊補了那邊忘記」的落差。**這次的教訓**：手刻 DOM 改寫成元件化實作時，容易漏掉一些「看起來像裝飾、實際上是特定環境才需要」的細節（這次是 z-index），改寫時應該逐一核對原始樣式的每一條規則是否都有對應保留，而不是只憑記憶重寫視覺上「看起來一樣」的部分。
