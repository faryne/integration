...
6. ~~**游標在表格內按 Enter 完全沒反應**~~ ✅ **已修（折衷方案，2026-08-18）**。跟圖片一樣有 Enter 不斷行的問題，但表格沒有像圖片那種「點一下選取整個節點」的手勢（表格走 `CellSelection`，不是 `NodeSelection`），跟圖片完全對等的做法（選取整個表格才斷行）需要先做一個新手勢，範圍超出這次修復。跟 Faryne 討論後定案折衷方案：只有游標在**最後一個 cell 的文字尾端**時 Enter 才在表格後面插入新段落，其他情況（不是最後一個 cell、或在文字中間）維持原本「沒反應」的行為，不會在編輯中間任何一個 cell 時被誤觸。詳見 [所見即所得編輯器notion-like分析_定案版.md](所見即所得編輯器notion-like分析_定案版.md) 的「已知 Bug 記錄」第 19 項。
7. **（待處理，非 bug，記錄需求）表格缺少「選取整個表格」的手勢**——第 6 項討論時發現的缺口：圖片可以直接點一下選取整個節點（`contenteditable=false`），表格沒有對應的東西，導致「選中整個表格再做某個操作」（斷行、刪除、複製等）目前都做不到，只能一個一個 cell 操作。Faryne 認為這個問題最終還是要面對，但範圍是新功能（要設計手勢／視覺回饋），不是這次的 bug 修復範圍，先記錄下來，等真的要處理時再展開。
...
**狀態**：項目 1、2、3、5、6 已修（項目 1 是項目 5 的副作用一併解決；過程中挖出新的已知 Bug 第 15 項——空白引用/清單行按 Backspace 沒有跳出格式，已修但「跳出格式後的合併」待人工複測）。項目 4 是功能請求、項目 7 是新發現的需求，都尚未排優先序，等真的要處理時再展開。
