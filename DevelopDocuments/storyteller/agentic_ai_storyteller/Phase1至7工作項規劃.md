...
- **選字右鍵觸發 AI 指令（`/rewrite` 等）**：2026-08-30 提出，規劃讓使用者選取文字後直接右鍵點 `/rewrite` 等選項送進 AI 助理處理，不用再手動複製貼上；已查證 `selected_content` 目前是半成品未接線、右鍵選單會收合選取範圍等技術限制，並確認存檔前送出鈕已免費鎖死、右鍵選單則需額外補判斷，見 [選字右鍵觸發AI指令規劃_2026-08-30.md](選字右鍵觸發AI指令規劃_2026-08-30.md)。**不要主動實作**，等使用者明確下指示才整理丟給 Codex。
- **MCP 方法更新（搜尋取代＋PATCH）**：2026-08-29 提出的 `storyteller_search_replace_story`／`_lore`（RE2、無 dry_run、一律大小寫敏感）＋2026-08-30 新增的 `storyteller_patch_story`／`_lore`（獨立於 `upsert_` 之外的局部更新工具，讀出＋合併不改既有 `UpdateStory`/`UpdateLore` 語意），兩者規格都已定案，見 [MCP方法更新規劃_2026-08-29.md](MCP方法更新規劃_2026-08-29.md)。**不要主動實作**，等使用者安排時間才動工（自己做或交給 Codex 由使用者決定）。
