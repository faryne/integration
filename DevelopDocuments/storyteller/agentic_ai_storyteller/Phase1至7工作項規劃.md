...
- ~~**提案（Proposal）否決後的意見回饋機制**~~：2026-08-29 已完成（Codex 實作＋review 通過），見 [提案否決回饋機制_2026-08-28.md](提案否決回饋機制_2026-08-28.md)。
- ~~**回覆／否決提案的摘要點擊展開原始內容**~~：2026-08-30 已完成（Codex 實作＋review 通過，改存參照不複製完整內容），見 [回覆與否決摘要點擊展開_2026-08-29.md](回覆與否決摘要點擊展開_2026-08-29.md)。
- ~~**Agentic 對話改背景執行＋輪詢＋重啟安全**~~：2026-08-30 已完成（Codex 實作＋review 通過，含 `-race` 驗證），見 [背景執行與重啟安全_2026-08-29.md](背景執行與重啟安全_2026-08-29.md)。
- ~~**AI 助理輸入框高亮與版面問題**~~：2026-08-30 提出的 4 個問題全部完成，見 [輸入框高亮與版面問題_2026-08-30.md](輸入框高亮與版面問題_2026-08-30.md)。面板高度／輸入框字體兩項先完成；@story/@lore 候選插入游標跑掉、slash command 語法高亮兩項交給 Codex 實作＋review 通過（含 `-race` 以外的 `tsc -b` 驗證＋實機瀏覽器測試）。額外修正兩個 review 時發現、原規劃沒提到的問題：(1) slash command 高亮沒驗證是不是真的存在的 skill／Agent 名稱，`/fuck` 這種亂打也會被誤標成合法指令，改成跟 `handleSend()` 送出邏輯一致的驗證；(2) 輸入框文字全透明的技巧會連注音等 IME 組字階段的原生候選底線一起藏起來，改成組字中暫時顯示真正文字、藏起 overlay，選字結束再切回。
- ~~**選字右鍵觸發 AI 指令（`/rewrite` 等）**~~：2026-08-30 提出，2026-08-31 完成（Codex 實作＋review 通過，含右鍵選單、Bubble Menu、`go test -race`／`tsc -b` 驗證）。選取文字後右鍵/浮動工具列點 skill、跳出額外需求彈窗，送出後把內容展開到 AI 助理輸入框（比照回覆/否決的引用卡片做法，不直接呼叫 API），使用者確認後自己按送出；徹底移除 `custom_chapter` 模式（`/custom` 一律用 `custom_selection`）；實測發現並修掉 `selection_start`/`selection_end` 驗證 bug，順帶把這兩個名存實亡的欄位整個拿掉。規格見 [選字右鍵觸發AI指令規劃_2026-08-30.md](選字右鍵觸發AI指令規劃_2026-08-30.md)。
- ~~**MCP 方法更新（搜尋取代＋PATCH）**~~：2026-08-31 已完成（Codex 實作＋review 通過，含 `go test -race`）。`storyteller_search_replace_story`／`_lore`（RE2、無 dry_run、一律大小寫敏感、圖片故事只動每頁 description）＋`storyteller_patch_story`／`_lore`（獨立於 `upsert_` 之外的局部更新工具，讀出＋合併不改既有 `UpdateStory`/`UpdateLore` 語意），見 [MCP方法更新規劃_2026-08-29.md](MCP方法更新規劃_2026-08-29.md)。
- **Skill 模式改用唯讀工具呼叫**：2026-08-31 提出並定案，起因是單輪 skill（`/rewrite` 等）沒有工具呼叫能力，多引用幾篇故事容易撞到 60000 字上限；方向是 `runAgent`/`RunLoreAgent` 有 `@` 引用時改走 `RunAgentLoop` 但只給唯讀工具（沒有引用或 provider 是 Gemini 時維持原本單次呼叫，一次解決延遲跟 Gemini 不支援 tools 的問題），完全不給寫入工具，保留 skill 不用走提案確認 UI 的既有保證。已經過 Codex 審視（抓到 context 注入、Gemini 相容性、`FinishReason` 遺失、`storyteller_list_projects` 排除、prompt 文案整理這幾個實作坑，都已補進規格）。**規格已定案，等排時間交給 Codex 實作。**
- **Agentic 多輪對話的人設風格隔離**：2026-08-31 提出並定案，起因是換人設（`/<Agent 名稱>`）只在系統層級「不沿用」，但 conversation history 裡不同人設寫的歷史內容沒有標記，模型可能延續前一輪別的人設的語氣，導致風格夾雜。方向是在歷史訊息裡標記「這一輪是哪個人設寫的」，並在 system prompt 明確指示模型歷史只當事實延續參考、語氣要照當下人設走，不保證 100% 隔絕但比完全沒提示好。兩者規格都在同一份文件：[Skill改用唯讀工具呼叫規劃_2026-08-31.md](Skill改用唯讀工具呼叫規劃_2026-08-31.md)。**規格已定案，等排時間交給 Codex 實作。**
