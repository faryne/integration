...
- **Skill 模式改用唯讀工具呼叫**：2026-08-31 提出並定案，起因是單輪 skill（`/rewrite` 等）沒有工具呼叫能力，多引用幾篇故事容易撞到 60000 字上限；方向是 `runAgent`/`RunLoreAgent` 有 `@` 引用時改走 `RunAgentLoop` 但只給唯讀工具（沒有引用或 provider 是 Gemini 時維持原本單次呼叫，一次解決延遲跟 Gemini 不支援 tools 的問題），完全不給寫入工具，保留 skill 不用走提案確認 UI 的既有保證。已經過 Codex 審視（抓到 context 注入、Gemini 相容性、`FinishReason` 遺失、`storyteller_list_projects` 排除、prompt 文案整理這幾個實作坑，都已補進規格），規格見 [Skill改用唯讀工具呼叫規劃_2026-08-31.md](Skill改用唯讀工具呼叫規劃_2026-08-31.md)。**規格已定案，等排時間交給 Codex 實作。**
