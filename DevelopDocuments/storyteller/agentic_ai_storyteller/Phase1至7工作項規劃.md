...
- **回覆／否決提案的摘要點擊展開原始內容**：2026-08-29 提出，原本想做「跳轉到原訊息」，後來發現完整內容其實已經存在該則訊息自己的 metadata 裡（`reply_content`，本來是給重送功能用的），改成點擊後原地展開/收合即可，不用跳轉，範圍也小很多。設計細節見 [回覆與否決摘要點擊展開_2026-08-29.md](回覆與否決摘要點擊展開_2026-08-29.md)。**不要主動實作**，等使用者明確下指示才整理丟給 Codex。
- **Agentic 對話改背景執行＋輪詢＋重啟安全**：2026-08-29 討論定案，起因是 HTTP client 逾時斷線會讓已經生成/正在生成的回應報銷，四家 provider 只有三家有官方 async 機制、Claude 沒有，決定改成後端自己用 goroutine 背景跑＋前端輪詢，重啟前要老實等所有背景 routine 跑完（不設等待上限），這套追蹤機制之後也要給重要的 crontab 排程共用。範圍明顯比其他項目大（牽動 HTTP handler 回應時機、`main.go` 關機流程、前端輪詢邏輯），設計細節見 [背景執行與重啟安全_2026-08-29.md](背景執行與重啟安全_2026-08-29.md)。**不要主動實作**，等使用者明確下指示才整理丟給 Codex。
