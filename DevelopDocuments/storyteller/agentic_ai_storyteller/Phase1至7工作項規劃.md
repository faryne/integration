...
- **選字右鍵觸發 AI 指令（`/rewrite` 等）**：2026-08-30 提出，2026-08-31 定案，規劃讓使用者選取文字後右鍵點 skill、跳出額外需求彈窗，送出後把內容展開到 AI 助理輸入框（比照回覆/否決的引用卡片做法，不直接呼叫 API），使用者確認後自己按送出。順帶決定徹底移除 `custom_chapter` 模式（沒有章節邊界資料結構，`/custom` 一律用 `custom_selection`）。規格見 [選字右鍵觸發AI指令規劃_2026-08-30.md](選字右鍵觸發AI指令規劃_2026-08-30.md)，已交給 Codex 實作中。
