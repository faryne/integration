...
const APPEARANCES = Object.keys(
  storytellerThemeTokens,
) as StorytellerAppearance[];
export function checkStorytellerContrast(): ContrastCheckResult[] {
  const results: ContrastCheckResult[] = [];

  for (const appearance of APPEARANCES) {
    const raw = storytellerThemeTokens[appearance];
    const tokens = toStorytellerSemanticTokens(raw);
    const mode = storytellerAppearanceMeta[appearance].mode;
    const common = {
      appearance,
      appearanceLabel: storytellerAppearanceMeta[appearance].label,
      mode,
    };
    ...
    pushResult(results, {
      ...
      category: "button",
      label:
        "MuiButton containedPrimary：textPrimary / 30% accentMain 疊 surfaceRaised",
      foreground: tokens.textPrimary,
