...
import Ajv from "ajv"
import {type CrawlerExecRequest, useCrawlerExec} from "@/apis/tools/crawler_exec.ts";
...
export function CrawlerIndex() {
    ...
    return (
        <Stack direction={"column"} spacing={1}>
            <TextField label={"請輸入要抓取的網址"} size={"medium"} fullWidth placeholder={"要抓取的網址"} value={input.uri} type={"url"} onChange={(e) => setInput(o => ({...o, uri: e.target.value}))} />
            <Box>
                ...
                <Table>
                    ...
                    <TableBody>
                        {Object.entries(schema.items.properties as Record<string, any>).map(([key, value]) => (
                            <TableRow key={`description-${key}`}>
                                <TableCell>{key}</TableCell>
                                <TableCell>{value.title}</TableCell>
                                <TableCell>{schema.items.required.indexOf(key) >= 0 ? "必填" : "選填"}</TableCell>
                                <TableCell>{value.type}</TableCell>
                                <TableCell>{typeof value.examples !== "undefined" ? (value.examples as string[]).join("、") : "-"}</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            ...
            <JsonEditor
                ...
                schema={schema}
                onValidationError={(errors: never[]) => setSubmitButton(errors.length > 0)}
            />
            <Divider />
            <Box textAlign={"center"}>
                <Button disabled={submitButton} variant={"contained"} onClick={() => {
                    console.log(input)
