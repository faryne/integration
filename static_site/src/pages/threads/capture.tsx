    ...
    return (
        <>
            <Stack direction={"column"} spacing={2}>
                <TextField label={"請輸入要抓取的網址"} size={"medium"} fullWidth placeholder={"要抓取的網址"} value={uri} type={"url"} onChange={(e) => setUri(e.target.value)} />
                ...
                {captureThread.data?.img && <>
                  <Card variant={"outlined"}>
                    <CardContent>
                      <img src={`data:image/png;base64, ${captureThread?.data?.img ?? ""}`} />
                    </CardContent>
                    <CardActions>
                      <Button onClick={() => {
                          const link = document.createElement('a');
                          link.href = `data:image/png;base64, ${captureThread?.data?.img ?? ""}`;
                          link.download = btoa(uri)
                              .replace(/\+/g, '-')  // 將 + 換成 -
                              .replace(/\//g, '_')  // 將 / 換成 _
