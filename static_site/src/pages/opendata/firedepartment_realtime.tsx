...
import {Typography, Table, TableHead, TableBody, TableRow, TableCell, Stack} from "@mui/material";
    ...
    return (
        <>
            <Stack direction={"column"} spacing={2}>
                {s.data && Object.entries(s?.data.data ?? {}).map(([k, v]) => (
                    <>
                        <Typography variant={"h4"}>{TWAreaMappings[k as keyof typeof TWAreaMappings]}</Typography>
                        <Table>
                            <TableHead>
                                <TableRow>
                            ...
                            <TableBody>
                                {v.map((e) => (
                                    <TableRow>
                                        <TableCell>{dayjs(e.timestamp*1000).format("YYYY-MM-DD HH:mm:ss")}</TableCell>
                                        <TableCell>{e.endpoint_info} {(e.lng && e.lat) && `(${e.lat}, ${e.lng})` || ""}</TableCell>
                                        <TableCell>{e.event_type} - {e.sub_type}</TableCell>
